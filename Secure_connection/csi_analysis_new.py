...
macs_filter = [
    # "e0:6d:17:3f:66:b8",  # 12pro
    # "9c:fc:28:a8:61:df", # jin 11
    "f0:2f:4b:0b:90:67"  # macbook
]
...
def read_csi(mac_need, f, offset, max_csi_frames: int = None, skipped_frames: int = None):
    csi_frames = []
    macaddr = []
        ...
        if tmp:
            ...
            if tmp == pattern[0:1]:
                ...
                if tmp == pattern[1:]:
                    ...
                    if mac_need and csi.mac not in mac_need:
                        continue
...
def csi_analyzer(mac_need, path, num_frame, flag):

    with open( path, "rb") as f:
        csi_frames = read_csi(mac_need, f, 0, num_frame, 0)
        # print(len(csi_frames))
        # print(np.shape(csi_frames[1].amplitude[0,0,:]))
        sub_num_a = np.shape(csi_frames[0].amplitude[0,0,:])[0]
        sub_num_p = np.shape(csi_frames[0].phases[0,0,:])[0]
    ...
    # print('yes')
    '''
    csi_phase_buff = np.zeros((no_frames,no_sub))
    csi_phase_grad = np.zeros((no_frames,no_sub))

    for i in range(no_frames):
        csi_phase_buff[i,:] = np.unwrap(csi_phase[i,:])
    ...
    frame_phase_std_selected = scoretable_phase_grad_std_sorted[0:200,:]
    csi_phase_normalized_new = np.zeros((no_frames,117))
    # print(scidx_40mhz_csi_no_pilot)
    for i in range(no_frames):
        n = 0
        for j in scidx_40mhz_csi_no_pilot:
            csi_phase_normalized_new[i,j+58] = csi_phase_normalized[i,n]
            n = n+1

    # print(scidx_40mhz_csi_no_pilot)
    # print(csi_phase_normalized_new[12,:])
    
    frame_no = [-53, -25, -11, -1, 1, 11, 25, 53]
    frame_no = [x+58 for x in frame_no]

    for i in range(no_frames):
        csi_phase_normalized_new[i,:] = remove_fill(csi_phase_normalized_new[i,:], frame_no)
    
    #print(csi_phase_normalized_new[12,:])
    #print(np.shape(csi_phase_normalized_new))
    
    phase_error = np.zeros(np.shape(csi_phase_normalized_new))
    

    for i in range(no_frames):
        z = (csi_phase_normalized_new[i,57] + csi_phase_normalized_new[i,59])/2
        # z = 0
        phi1 = csi_phase_normalized_new[i,0]-z
        phi2 = csi_phase_normalized_new[i,114]-z
        lamda1 = phi1 / (2*np.pi*-58)
        lamda2 = phi2 / (2*np.pi*58)
        lamda = (lamda1+lamda2)/2
    
        for j in range(115):
            phase_error[i,j] = (csi_phase_normalized_new[i,j]-(2*np.pi*lamda*(j-58)+z))

    for i in range(np.shape(phase_error)[0]):
        # phase_error[i,:] = hampel(phase_error[i,:],3,1)
        phase_error[i,:] = running_mean(phase_error[i,:],3)
...
if __name__ == "__main__":
    ...
    df, csi_amplitude_processed= csi_analyzer(path= '/var/log/csi.dat', num_frame=10)
    
    # for i in range(np.shape(csi_phase)[0]):
    #     plt.figure(1)
    #     plt.plot(scidx_40mhz_csi_no_pilot,csi_phase[i,:])
    #     plt.show()
    ...
    # for i in range(np.shape(csi_amplitude)[0]):
    #     plt.plot(scidx_40mhz_csi_no_pilot,csi_amplitude[i,:])
    # plt.show()
