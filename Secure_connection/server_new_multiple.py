...
def main():
    ...
    mac_need1 = [
            ...
            '9c:b6:d0:db:ce:35', # dell
    ]
    mac_need1 = [mac.lower() for mac in mac_need1]

    mac_need2 = [
            
            '3c:58:c2:68:15:50'  # lenovo
    ]
    mac_need2 = [mac.lower() for mac in mac_need2]
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        ...
        flag, addr_client1 = s.recvfrom(1024)
        print('Connected by', addr_client1)
        
        
        flag, addr_client2 = s.recvfrom(1024)
        print('Connected by', addr_client2)
        ...
        t1 = Thread(target=authentication_new, args=(p,s,clientPubKey,serverPvKey,mac_need1,'user1', addr_client1))
        t2 = Thread(target=authentication_new, args=(p,s,clientPubKey,serverPvKey,mac_need2,'user2', addr_client2))
...
def authentication_new(p, s, clientPubKey, serverPvKey, mac_need, user_flag, addr_client):
    ...
    flag, addr_client = s.recvfrom(1024)
    ...
    addr1 = ['192.168.3.188', '192.168.3.108']
    addr2 = ['192.168.3.98']
    addr3 = '' # spoofer addr
    ...
    if user_flag == 'user1':
        addrmid = addr1
        #addr = addr1[0]
    elif user_flag == 'user2':
        addrmid = addr2
        #addr = addr2[0]
        
    addr_get = addr_client[0]
    ...
    while True:
        ...
        if file_exist_profile == True:
            ...
            df_train, matrix = csi_analysis_new.csi_analyzer(mac_need, path= '/home/nss/profile.dat',num_frame= 1500, flag='train')
        ...
        while True:
                ...
                if file_exist_profile == True and initial == '0':
                    ...
                    if file_exist_test == True:
                        if i%50 == 0:
                            ...
                            df_test, matrix = csi_analysis_new.csi_analyzer(mac_need, path = '/var/log/csi.dat', num_frame = 50, flag='test')
            ...
            # flag, plain_text, signature = [i for i in s.recv(1024).split(b'$$$$$')]
            flag, plain_text, signature = [i for i in message.split(b'$$$$$')]

            # if addr != addrmid:
            if addr_get not in addrmid:
                continue
