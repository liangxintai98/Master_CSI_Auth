    ...
    if __name__ == "__main__":
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while True:
                ...
                s.sendto(b'$$$$$'.join([b"TEXT", text, signature]),(HOST,PORT))
                print('message sent!')
                ...
                f = open("cpu_client_time_symm.txt", "w")
                for d in cpu_rec:
                    f.write(f"{d}\n")
                f.close()
...
if __name__ == "__main__":
    ...
    main()
    # current, peak = tracemalloc.get_traced_memory()
    # print(f"Current memory usage is {current / 10**6}MB; Peak was {peak / 10**6}MB")_new
    # tracemalloc.stop()
